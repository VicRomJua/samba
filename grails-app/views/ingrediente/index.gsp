			...
			function jconfirm(pregunta,mensaje,id){
                swal({
                    ...
                    buttonsStyling: true
                     ...
                     location.href = "${request.contextPath}/ingrediente/remove/"+id;
                });
            }
    ...
    <body>
		...
		<div class="x_panel">
	        ...
	        <div class="nav_topbar" role="navigation">
	            <ul>
	                ...
	                <div class="col-xs-6 col-xs-offset-2" style="float:right;padding: 0px;">
						<g:if test="${ingredienteCount != 0 || session.getAttribute('ingrediente_search_word')}">
						<g:form action="index" method="POST" name="search_form">
							<div class="input-group">
								<div class="input-group-btn search-panel">
									<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
										<span id="search_concept">${session.getAttribute('ingrediente_search_rows')} Registros</span> <span class="caret"></span>
									</button>
								...
								<input type="hidden" name="search_rows" id="search_rows" value="${session.getAttribute('ingrediente_search_rows')}">
								<input type="hidden" name="search_clean" id="search_clean" value="0">
								<input type="text" class="form-control" name="search_word" id="search_word" placeholder="Palabra a buscar..." value="${session.getAttribute('ingrediente_search_word')}">
	        ...
	        <div id="list-ingrediente" class="content scaffold-list" role="main">
	            ...
	            %{--
	            <f:table collection="${ingredienteList}" />
				--}%
				
				<table>
					<g:if test="${ingredienteCount != 0}">
	                <thead>
	                    <tr>
							<th style="width:100px" class="sortable"><a href="#">${message(code: 'ingrediente.archivo_Foto.label', default: 'Photo')}</a></th>
	                        <g:sortableColumn property="nombre" title="${message(code: 'ingrediente.nombre.label', default: 'Name')}" style="width:40%"/>
	                        <g:sortableColumn property="unidadMedicion" title="${message(code: 'ingrediente.unidadMedicion.label', default: 'Unity')}" />
	                        <th style="width:100px" class="sortable"><a href="#">Operaciones</a></th>
	                    </tr>
	                </thead>
					</g:if>
	                <tbody>
	                    <g:each in="${ingredienteList}" var="item" status="i">
	                    ...
	                    <g:if test="${ingredienteCount == 0}">
							<tr>
								<td colspan="4" style="text-align:center"></br></br><img src="${request.contextPath}/assets/empty/ingredientes.png" border="0px"/></br><span style="font-size:18px;color:#b1b1b1;"><g:message code="default.table.empty.label" args="[entitiesName]"/></span></br></br></td>
							</tr>
	                    </g:if>
	                </tbody>
	            </table>
	         ...
	         <div class="pagination">
	                <g:paginate total="${ingredienteCount ?: 0}" />
	         </div>
	    </div>

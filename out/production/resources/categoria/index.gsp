			...
			function jconfirm(pregunta,mensaje,id){
                swal({
                    ...
                    buttonsStyling: true
                     ...
                     location.href = "${request.contextPath}/categoria/remove/"+id;
                });
            }
    ...
    <body>
		...
		<div class="x_panel">	
	        ...
	        <div class="nav_topbar" role="navigation">
	            <ul>
	                ...
	                <div class="col-xs-6 col-xs-offset-2" style="float:right;padding: 0px;">
						<g:if test="${categoriaCount != 0 || session.getAttribute('categoria_search_word')}">
						<g:form action="index" method="POST" name="search_form">
							<div class="input-group">
								<div class="input-group-btn search-panel">
									<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
										<span id="search_concept">${session.getAttribute('categoria_search_rows')} Registros</span> <span class="caret"></span>
									</button>
								...
								<input type="hidden" name="search_rows" id="search_rows" value="${session.getAttribute('categoria_search_rows')}">
								<input type="hidden" name="search_clean" id="search_clean" value="0">
								<input type="text" class="form-control" name="search_word" id="search_word" placeholder="Palabra a buscar..." value="${session.getAttribute('categoria_search_word')}">
	        ...
	        <div id="list-categoria" class="content scaffold-list" role="main">
				...
				<table>
					<g:if test="${categoriaCount != 0}">
	                <thead>
	                    <tr>
							<th style="width:100px" class="sortable"><a href="#">${message(code: 'categoria.archivo_Foto.label', default: 'Photo')}</a></th>
	                        <g:sortableColumn property="nombre" title="${message(code: 'categoria.nombre.label', default: 'Name')}"/>
	                        <th style="width:100px" class="sortable"><a href="#">Operaciones</a></th>
	                    </tr>
	                </thead>
					</g:if>
	                <tbody>
	                    <g:each in="${categoriaList}" var="item" status="i">
	                        <tr class="${(i % 2) == 0 ? 'even' : 'odd'}">
	                            <td style="width:100px;">
									<g:if test="${item.archivo_Foto}">
									<center>
										<a href="javascript:showImage('${item.nombre.replaceAll("'", "").trim()}','${item.archivo_Foto}');"><img src="${request.contextPath}/assets/mes-icons/${item.archivo_Foto}" border="0px" Style="width:50px;height:50px;"/></a>
									</center>
									</g:if>
								</td>
	                    ...
	                    <g:if test="${categoriaCount == 0}">
							<tr>
								<td colspan="4" style="text-align:center"></br></br><img src="${request.contextPath}/assets/empty/categorias.png" border="0px"/></br><span style="font-size:18px;color:#b1b1b1;"><g:message code="default.table.empty.label" args="[entitiesName]"/></span></br></br></td>
							</tr>
	                    </g:if>
	                </tbody>
	            </table>

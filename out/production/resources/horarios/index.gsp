...
<html>
            ...
            function jconfirm(pregunta,mensaje,id){
                swal({
                    ...
                    buttonsStyling: true
                     ...
                     location.href = "${request.contextPath}/horarios/remove/"+id;
                });
            }
        </script>
    </head>
    <body>
		...
		<!-- .breadcrumb-box -->
        <a href="#list-horarios" class="skip" tabindex="-1"><g:message code="default.link.skip.label" default="Skip to content&hellip;"/></a>
					...
					<div class="input-group">
						<div class="input-group-btn search-panel">
							<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
								<span id="search_concept">${session.getAttribute('horarios_search_rows')} Registros</span> <span class="caret"></span>
							</button>
						...
						<input type="hidden" name="search_rows" id="search_rows" value="${session.getAttribute('horarios_search_rows')}">
						<input type="hidden" name="search_clean" id="search_clean" value="0">
						<input type="text" class="form-control" name="search_word" id="search_word" placeholder="Palabra a buscar..." value="${session.getAttribute('horarios_search_word')}">
        ...
        <div id="list-horarios" class="content scaffold-list" role="main">
            <g:if test="${flash.message}">
                <div class="message" role="status" style="float:left;width:97%">${flash.message}</div>
            </g:if>
            <f:table collection="${horariosList}" />

            <div class="pagination">
                <g:paginate total="${horariosCount ?: 0}" />
            </div>
        </div>
    </body>
</html>

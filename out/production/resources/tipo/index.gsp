			...
			function jconfirm(pregunta,mensaje,id){
                swal({
                    ...
                    buttonsStyling: true
                     ...
                     location.href = "${request.contextPath}/tipo/remove/"+id;
                });
            }
    ...
    <body>
		...
		<div class="x_panel">			
	        ...
	        <div class="nav_topbar" role="navigation">
	            <ul>
	                ...
	                <div class="col-xs-6 col-xs-offset-2" style="float:right;padding: 0px;">
						<g:if test="${tipoCount != 0 || session.getAttribute('tipo_search_word')}">
						<g:form action="index" method="POST" name="search_form">
							<div class="input-group">
								<div class="input-group-btn search-panel">
									<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
										<span id="search_concept">${session.getAttribute('tipo_search_rows')} Registros</span> <span class="caret"></span>
									</button>
								...
								<input type="hidden" name="search_rows" id="search_rows" value="${session.getAttribute('tipo_search_rows')}">
								<input type="hidden" name="search_clean" id="search_clean" value="0">
								<input type="text" class="form-control" name="search_word" id="search_word" placeholder="Palabra a buscar..." value="${session.getAttribute('tipo_search_word')}">
	        ...
	        <div id="list-tipo" class="content scaffold-list" role="main">
				...
				<table>
					<g:if test="${tipoCount != 0}">
	                <thead>
	                    <tr>
							<th style="width:100px" class="sortable"><a href="#">${message(code: 'tipo.archivo_Foto.label', default: 'Photo')}</a></th>
	                        <g:sortableColumn property="nombre" title="${message(code: 'tipo.nombre.label', default: 'Name')}"/>
	                        <g:sortableColumn property="superTipo" title="${message(code: 'tipo.superTipo.label', default: 'Kinds')}"/>
	                        <th style="width:100px" class="sortable"><a href="#">Operaciones</a></th>
	                    </tr>
	                </thead>
					</g:if>
	                <tbody>
	                    <g:each in="${tipoList}" var="item" status="i">
	                    ...
	                    <g:if test="${tipoCount == 0}">
							<tr>
								<td colspan="4" style="text-align:center"></br></br><img src="${request.contextPath}/assets/empty/tipos.png" border="0px"/></br><span style="font-size:18px;color:#b1b1b1;"><g:message code="default.table.empty.label" args="[entitiesName]"/></span></br></br></td>
							</tr>
	                    </g:if>
	                </tbody>
	            </table>
